...
这套边界让 Agent 既能推进工作，又不会因为模型误判直接修改求职数据。无 API Key 时，本地策略可识别常见求职问法，按计划连续调用看板、职业档案、行动项和训练等工具；对于简历任务会先让用户选择具体简历，再完成结构化诊断或生成事实保真的可编辑草稿，确认后才另存为新版本。配置模型后，再由模型负责更开放的理解和表达。
