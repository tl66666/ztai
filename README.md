...
- **ReAct Agent 架构**：AI 教练不是固定流程的工作流，而是基于 ReAct 框架的真正 Agent。LLM 自主思考、自主选择 11 个工具、自主决定调几次、自主判断何时完成。9 个内部工具聚焦求职核心流程，2 个外部工具联网获取实时信息。有 API Key 时由大模型决策，无 Key 时降级为关键词意图识别。
...
- **11 个工具**：简历分析、岗位匹配、面试题获取、回答评估、JD 解析、薪资评估、简历查询、投递查询、追问用户、联网搜索、网页抓取。9 个内部工具聚焦求职核心流程，2 个外部工具（联网搜索 + 网页抓取）获取实时面试经验和公司评价。知识类问题由 LLM 自身能力直接回答，实时信息通过外部搜索获取。
