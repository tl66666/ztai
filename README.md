...
- **ReAct Agent 架构**：AI 教练不是固定流程的工作流，而是基于 ReAct 框架的真正 Agent。LLM 自主思考、自主选择 9 个工具、自主决定调几次、自主判断何时完成。知识类问题由 LLM 自身能力直接回答，不需要外部搜索 API。有 API Key 时由大模型决策，无 Key 时降级为关键词意图识别。
...
- **9 个工具**：简历分析、岗位匹配、面试题获取、回答评估、JD 解析、薪资评估、简历查询、投递查询、追问用户。全部聚焦求职场景，知识类问题由 LLM 自身能力直接回答。
