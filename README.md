...
这套边界让 Agent 既能推进工作，又不会因为模型误判直接修改求职数据。无 API Key 时，用户可以点击 Agent 中的“带我开始”，或输入“我是新用户，带我开始使用这个求职系统”；本地策略会读取看板、职业档案、行动项和训练记录，按当前缺口给出可点击的目标档案、简历录入、JD 匹配、面试训练或投递看板入口。对于简历任务会先让用户选择具体简历，再完成结构化诊断或生成事实保真的可编辑草稿，确认后才另存为新版本。配置模型后，再由模型负责更开放的理解和表达。
...
- Agent：21 个结构化工具、有界编排循环、本地确定性多工具规划、分层记忆、上下文重建、确认式动作、幂等回执。
