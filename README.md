...
[在线项目展示](https://tl66666.github.io/ztai/static/showcase.html) · [用户指南](docs/USER_GUIDE.md) · [架构说明](docs/ARCHITECTURE.md) · [测试指南](docs/TESTING.md) · [版本记录](CHANGELOG.md)
...
这套边界让 Agent 既能推进工作，又不会因为模型误判直接修改求职数据。无 API Key 时，本地规则策略仍能查询数据、计算匹配、生成确定性计划和操作提案，并明确标注本地模式；配置模型后，再由模型负责更灵活的理解和表达。
...
未配置 Key 时不会伪造“已调用大模型”。界面会显示本地规则模式，仍可完成结构化查询、基础分析、面试流程和安全操作提案，但生成式表达能力有限。
...
- Agent：有界编排循环、严格工具参数、分层记忆、上下文重建、确认式动作、幂等回执。
