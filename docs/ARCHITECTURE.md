...
远程模型优先使用供应商原生 `tool_calls`；兼容模式只接受严格 JSON。编排器有轮数、总时长、工具超时和重复调用预算，达到边界后使用已取得的可靠结果结束，不无限自循环。

工具参数由 JSON Schema 校验。读取工具通过领域服务或受限查询返回数据；联网搜索和网页抓取仅访问公开网络，拒绝回环、内网地址和超大响应。模型看不到可写的 `user_id`。
