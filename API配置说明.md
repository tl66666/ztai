...
- 不写入浏览器 localStorage；
- 不写入仓库；
- 仅保存在当前电脑的运行目录，文件包含明文 Key。

该方式适合单用户本机使用。不要在已改为局域网或公网监听的未加密服务中提交 Key；不再使用时，在“模型配置”中留空保存即可清除本机文件。
...
- 页面配置未生效：检查 `output/runtime/ai-config.json` 是否可由当前用户读写，或改用环境变量后重新启动。
